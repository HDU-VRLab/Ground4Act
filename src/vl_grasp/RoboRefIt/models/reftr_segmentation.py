...
class RefTRSeg(RefTR):
    ...
    def forward(self, samples):
        ...
        confidence = pred_boxes[..., -1]  # 获取最后一个维度的数值作为置信度
        confidence =  confidence.cpu().detach().numpy().squeeze()  # 将张量转换为 numpy 数组
        # print(confidence) 
        # print("*****************************************************************")       
...
def build_reftr_seg(args):
    ...
    if args.reftr_type == 'transformer_single_phrase':
        ...
        if args.bert_model.split('-')[0] == 'roberta':
            ...
            # path = '/home/user/gjt/vl_grasp/logs/bert-base-uncased'
            lang_backbone = AutoModel.from_pretrained(args.bert_model_path)
